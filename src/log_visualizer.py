...
OUTPUT_DIR = "visualization_reports"
FIG_SIZE_LARGE = (20, 16)
FIG_SIZE_NORMAL = (18, 15)

def load_stats_from_json(filepath):
    print(f"Reading analysis data from: {filepath}")
    try:
...
def create_memory_plot(ax, stats):
    ...
    bars = sns.barplot(x="Tracker", y="Unique Items", data=df_mem, 
                      ax=ax, palette="Blues_r", hue="Mode", dodge=False)
    
    ax.set_title("Unique Items Tracked", fontsize=12, pad=20)
    ax.set_xlabel("Tracker Type")
    ax.set_ylabel("Count")
    ...
    ax.legend(title="Tracking Mode", fontsize=9, title_fontsize=10)
...
def create_error_rate_chart(ax, stats):
    ...
    labels = ['Success (2xx/3xx)', 'Client Errors (4xx)', 'Server Errors (5xx)']
    ...
    for bar, rate in zip(bars, rates):
        ...
        if rate > 5 and bar.get_label() in ['Client Errors (4xx)', 'Server Errors (5xx)']:
            bar.set_edgecolor('darkred')
            bar.set_linewidth(3)
            bar.set_alpha(0.9)
            ax.text(bar.get_x() + bar.get_width()/2., height + (max_rate * 0.05),
                   'HIGH!', ha='center', va='bottom', color='darkred', 
                   fontsize=9, fontweight='bold', rotation=0)
    
    ax.axhline(y=5, color='darkred', linestyle='--', linewidth=3, alpha=0.9, 
               label='5% Error Threshold (Warning Level)')
    
    ax.axhspan(5, max_rate * 1.3, alpha=0.15, color='red', label='High Error Zone')
    
    if server_error_rate > 5 or client_error_rate > 5:
        warning_text = 'High Error Rates Detected!'
...
def visualize_results(stats, output_filename_base="log_analysis_dashboard"):
    print("Creating visualizations...")
    
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    num_top_ips = len(stats["traffic_analysis"]["top_ips"])
    fig_size = FIG_SIZE_LARGE if num_top_ips > 20 else FIG_SIZE_NORMAL
    
    fig = plt.figure(figsize=fig_size)
    
    main_title = (f"Log Analysis Dashboard | "
                  f"Total Requests: {stats['summary']['total_requests']:,} | "
    ...
    print("  Plot 1/6: HTTP Status Distribution...")
    ...
    colors = [
        "mediumseagreen" if k.startswith("2") else
        "skyblue" if k.startswith("3") else
        "salmon" if k.startswith("4") else
        "lightcoral"
        for k in labels
    ]
    ...
    if total > 0:
        ...
        wedges, texts = ax1.pie(main_sizes, labels=main_labels, autopct=None,
                               startangle=90, colors=main_colors,
    ...
    print("  Plot 2/6: Top IP Addresses...")
    top_ips_data = stats["traffic_analysis"]["top_ips"]
    df_ips = pd.DataFrame(top_ips_data, columns=["IP Address", "Count"])
    
    if not df_ips.empty:
        sns.barplot(x="Count", y="IP Address", data=df_ips, ax=ax2, 
                   hue="IP Address", palette=COLOR_PALETTE + "_r", 
        ...
        if len(df_ips) > 0:
            ...
            if total_reqs > 0:
                ...
                if percentage > 10:
                    ax2.annotate(f'{percentage:.1f}% of total', 
                               xy=(top_ip_count, 0), 
                               xytext=(top_ip_count*1.1, 0),
                               arrowprops=dict(arrowstyle="->", color='darkred', lw=2),
                               fontsize=9, fontweight='bold')
    
    print("  Plot 3/6: Top URLs...")
    top_urls_data = stats["traffic_analysis"]["top_urls"]
    df_urls = pd.DataFrame(top_urls_data, columns=["URL Path", "Count"])
    
    if not df_urls.empty:
        df_urls["URL Path Short"] = df_urls["URL Path"].apply(
            lambda x: (x[:35] + '...') if len(x) > 38 else x
        )
        
        sns.barplot(x="Count", y="URL Path Short", data=df_urls, ax=ax3,
                   hue="URL Path Short", palette="mako_r", 
                   legend=False, dodge=False)
    ...
    if minutes_data:
        ...
        if parsed_data:
            ...
            if not df_minutes.empty:
                sns.lineplot(x="Minute", y="Count", data=df_minutes, 
                           ax=ax4, marker="o", color="darkorange", linewidth=2.5)
    ...
    if not df_methods.empty:
        sns.barplot(x="Method", y="Count", data=df_methods, ax=ax5,
                   hue="Method", palette=CATEGORICAL_PALETTE, 
                   legend=False, dodge=False)
    ...
    print("  Plot 6/6: Error Rate Analysis...")
    ...
    try:
        ...
        json_output = os.path.join(OUTPUT_DIR, f"dashboard_data_{timestamp}.json")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        print("Example: python log_visualizer.py log_analysis_report_20231201_143022.json")
        sys.exit(1)
    
    print("=" * 60)
    print("LOG ANALYSIS VISUALIZER")
    print("=" * 60)
